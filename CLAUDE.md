...
- **Presence ships dark: it is gated on `CONVEX_URL` in `js/presence.js`.** Empty (the current state) means no network, no error, no UI. The Convex deployment is not provisioned because the CLI login on this machine is stale and needs the user interactively. To bring it live: `npx convex login`, then `npx convex dev --once` in this project (creates the deployment, regenerates `convex/_generated/`, pushes `presence.ts`), then paste the deployment URL (`https://<name>.convex.cloud`, printed by the CLI and written to `.env.local`) into `CONVEX_URL`. The client wiring copies memes-site (`js/state.js` there): `ConvexHttpClient` from the pinned esm.sh build, string function names, one-shot queries polled every 2s (no websocket, no build step). Presence is view-only like Sim: nothing lands in the document, localStorage or the exports (remote visitors carry no `data-svg` role), the room key is an FNV-1a hash of the canonical YAML (so a shared `#d=` link is a shared room), and rows expire ~15s after the last heartbeat. `convex/_generated/` is gitignored (memes-site convention); the local copies were seeded from memes-site so imports resolve before the first codegen.
