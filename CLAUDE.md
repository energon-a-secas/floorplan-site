...
`make test` runs the Node tests (presence core). `make install` / `make convex` / `make deploy` /
`make login` manage the optional Visit-mode presence backend (see the presence gotcha before touching it).
...
| `js/presence.js`, `js/presence-core.js` | multiplayer presence in Visit mode (lazy-imported by `visit.js`): remote visitors on `#buildingLayer` as `.visitor--remote` with a name tag, an "N others here" pill, re-mount on `floorplan:board`; `presence-core.js` is the pure heartbeat/poll/stale logic (no DOM, no imports, Node-tested in `test/`). Backend: `convex/schema.ts` + `convex/presence.ts` (`heartbeat` upsert, `leave`, `others` fresher than 15s, lazy sweep). A no-op until `CONVEX_URL` in `presence.js` is set; see the presence gotcha |
...
| `js/tour.js` | first-visit spotlight tour (lazy-imported by `loadExample`'s firstRun path): six steps over the shell, one `.tour-spot` whose giant box-shadow is the dimmer, seen-flag in localStorage `floorplan-tour-v1` (view-only, never in the document); gated off for embed, readonly and `?sim=`, and the three header steps collapse to one beat when the mobile overflow menu holds them |
...
- **Presence is live, gated on `CONVEX_URL` in `js/presence.js`** (deployment `dev:adjoining-crane-592`, team `lucio`, provisioned 2026-08-29 and verified with two tabs seeing each other). An empty `CONVEX_URL` means no network, no error, no UI; to re-provision, `npx convex dev --once` here (the CLI is logged in), then put the printed `https://<name>.convex.cloud` URL into `CONVEX_URL`. The client wiring copies memes-site (`js/state.js` there): `ConvexHttpClient` from the pinned esm.sh build, string function names, one-shot queries polled every 2s (no websocket, no build step). Presence is view-only like Sim: nothing lands in the document, localStorage or the exports (remote visitors carry no `data-svg` role), the room key is an FNV-1a hash of the canonical YAML (so a shared `#d=` link is a shared room), and rows expire ~15s after the last heartbeat. `convex/_generated/` is gitignored (memes-site convention).
