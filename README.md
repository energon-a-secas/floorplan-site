...
- **Multiplayer visit** -- with the optional presence backend configured, people with the same document open (the same `#d=` link) see each other walking the building: translucent pixel visitors wearing their Pixeldoll look (or a seeded one) with a guest tag, and an "N others here" pill; ephemeral by design, a visitor fades about 15s after their last heartbeat, and none of it lands in the document or the exports; without the backend the feature stays off entirely
